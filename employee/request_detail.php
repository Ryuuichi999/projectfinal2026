    ...
    <div class="container-fluid">
        ...
        <div class="d-flex justify-content-end align-items-center gap-2 mb-3">
            ...
            <?php if (in_array($request['status'], ['pending','reviewing','need_documents'])): ?>
                <a href="approve_form.php?id=<?= $request['id'] ?>" class="btn btn-success"><i class="bi bi-check-circle"></i> ยืนยัน/อนุมัติ</a>
            <?php endif; ?>
            <?php if (!in_array($request['status'], ['approved','rejected'])): ?>
                <button class="btn btn-warning" type="button" data-bs-toggle="modal" data-bs-target="#requestDocsModal"><i class="bi bi-file-earmark-plus"></i> ขอเอกสารเพิ่ม</button>
                <button class="btn btn-danger" type="button" data-bs-toggle="modal" data-bs-target="#rejectModal"><i class="bi bi-x-circle"></i> ปฏิเสธ</button>
            <?php endif; ?>
        </div>
                ...
                <div class="card p-4 mb-4">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="detail-value"><?= date('d/m/Y H:i', strtotime($request['created_at'])) ?></div>
                        </div>
                        <div class="col-md-6">
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="detail-value">
                                ...
                                <span class="text-muted">(<?= $request['width'] * $request['height'] ?> ตร.ม.)</span>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="detail-label text-muted">ค่าธรรมเนียมประเมิน</div>
                            <div class="detail-value text-success fw-bold"><?= number_format($request['fee']) ?> บาท</div>
                        </div>
                        <div class="col-md-6">
                        ...
                        <div class="col-12">
                            ...
                            <div class="p-3 bg-light rounded mt-1"><?= nl2br(htmlspecialchars($request['description'])) ?></div>
                        </div>
                        <?php if (!empty($request['decision_note'])): ?>
                            <div class="col-12">
                                <div class="detail-label text-muted">บันทึกการตัดสินใจ</div>
                                <div class="p-3 bg-warning-subtle rounded mt-1"><?= nl2br(htmlspecialchars($request['decision_note'])) ?></div>
                            </div>
                        <?php endif; ?>
                    </div>
                </div>
                        ...
                        <div class="d-flex flex-column gap-2">
                            <?php while ($doc = $result_docs->fetch_assoc()): ?>
                                <div class="doc-item">
                                    ...
                                    <a href="<?= htmlspecialchars($doc['file_path']) ?>" target="_blank" class="btn btn-outline-primary btn-sm w-100">ดูเอกสาร</a>
                                </div>
                            <?php endwhile; ?>
                        </div>
                    <?php else: ?>
    ...
    <form method="post" class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div class="mb-2">
          ...
          <textarea name="note" class="form-control" rows="3" placeholder="เช่น โปรดแนบสำเนาบัตรประชาชนเพิ่มเติม หรือเอกสารยินยอมเจ้าของที่" required></textarea>
        </div>
      </div>
    ...
    <form method="post" class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div class="mb-2">
          ...
          <textarea name="note" class="form-control" rows="3" placeholder="โปรดระบุเหตุผลอย่างชัดเจน" required></textarea>
        </div>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">ยกเลิก</button>
